...
p_cutoffLP_L = vslider("cutoffLP_L[unit:Hz][style:knob][scale:log]", 20000, 20, 20000, 0.1) / (ma.SR / 2) : si.smoo;
p_cutoffHP_L = vslider("cutoffHP_L[unit:Hz][style:knob][scale:log]", 20, 20, 20000, 0.1) / (ma.SR / 2) : si.smoo;
p_cutoffLP_R = vslider("cutoffLP_R[unit:Hz][style:knob][scale:log]", 20000, 20, 20000, 0.1) / (ma.SR / 2) : si.smoo;
p_cutoffHP_R = vslider("cutoffHP_R[unit:Hz][style:knob][scale:log]", 20, 20, 20000, 0.1) / (ma.SR / 2) : si.smoo;
...
// Left channel
voice_L = _ : *(p_inputVolume) <: delayLoop_L * p_dryWet_L, *(1 - p_dryWet_L) :> *(p_outputVolume);
delayLoop_L = (+ : delay_L) ~ (*(p_feedback_L) : feedbackChain_L);
feedbackChain_L = aa.tanh1 : ve.sallenKeyOnePoleLPF(p_cutoffLP_L) : ve.sallenKeyOnePoleHPF(p_cutoffHP_L);
delay_L = de.sdelay(maxDelay, interpTime, p_delayTime_L);

// Right channel
voice_R = _ : *(p_inputVolume) <: delayLoop_R * p_dryWet_R, *(1 - p_dryWet_R) :> *(p_outputVolume);
delayLoop_R = (+ : delay_R) ~ (*(p_feedback_R) : feedbackChain_R);
feedbackChain_R = aa.tanh1 : ve.sallenKeyOnePoleLPF(p_cutoffLP_R) : ve.sallenKeyOnePoleHPF(p_cutoffHP_R);
